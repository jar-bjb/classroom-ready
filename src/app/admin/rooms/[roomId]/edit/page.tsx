    ...
    <>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 lg:py-10">
        ...
        <section className="rounded-3xl border border-border bg-card p-6 shadow-sm">
          ...
          <p className="mt-2 text-muted">Edit/hapus fasilitas dan komponen pemeriksaan khusus kelas ini. Untuk melihat hasil laporan, buka dari Dashboard.</p>
        </section>
                ...
                {section.items.map((item) => (
                  <div key={item.id} className="rounded-2xl border border-border bg-background p-4">
                    ...
                    <form action={deleteRoomComponent} className="mt-2">
                      <input type="hidden" name="roomId" value={room.id} />
                      <input type="hidden" name="itemId" value={item.id} />
                      <button className="inline-flex items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-bold text-rose-700"><Trash2 size={16} /> Hapus dari kelas ini</button>
                    </form>
                  </div>
                ))}
              </div>
